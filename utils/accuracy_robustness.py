...
def _corrupt_loop(eval_x, eval_y, rank, model, feat_per_step, mode, method_name=None):
    ...
    for i in range(steps):
        ...
        # support for 3 channel
        batch_mask = np.repeat(batch_mask[:,:,:,np.newaxis], 3, axis = 3)
...
def run_robustness_test(model, x, y, original_maps, names, task_name, feat_per_step,
    ...
    colors_max = ['tab:blue', 'tab:orange', 'tab:green', 'xkcd:crimson', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan', 'xkcd:indigo', 'xkcd:mint', 'xkcd:salmon', 'm']
    colors_min = ['#03797c', '#5b5aac', '#009fcb', '#676767', '#4285f4', '#141a30']
    markers = [".", ",", "o","v","^","<",">","s","p","P","*","h","H","+","x","X","D","d","|","_"]
    msizes = [8 for i in range(25)]
    ...
    x_ticks = np.linspace(0, CORRUPTION_MAX_RATIO*100, n_ticks)
    ...
    for idx, map_name in enumerate(plot_data_max):
        ...
        plt.plot(x_ticks, y, color=colors_max[idx],
                ...
                markevery=200 + np.random.randint(-5, 5),
                alpha=1)
    ...
    fig.text(0.5, 0.22, 'Features removed (%)', ha='center', va='center', fontsize=12)
    fig.text(0.05, 0.55, 'Change in output', ha='center', va='center', rotation='vertical', fontsize=12)
    legend = fig.legend(names + ['Random'], loc='lower center', fontsize=12, ncol=5, bbox_to_anchor=(0, 0, 1, 1),
    ...
    plt.savefig("robustness.eps", format = 'eps', facecolor=fig.get_facecolor(), transparent=True, bbox_inches = 'tight',pad_inches = 0)
    plt.show()
